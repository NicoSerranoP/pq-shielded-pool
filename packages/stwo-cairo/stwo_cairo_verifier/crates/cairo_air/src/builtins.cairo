...
use stwo_cairo_air::{PublicData, SegmentRange, components};
...
fn segment_start_ptr(segment_range: @Option<SegmentRange>) -> Span<u32> {
    let Some(segment_range) = segment_range else {
        panic!("missing builtin segment range");
    };
    [*segment_range.start_ptr.value].span()
}
#[derive(Drop)]
        ...
        if let Option::Some(component) = add_mod_builtin.as_snap() {
            component
                .evaluate_constraints_at_point(
                    ...
                    random_coeff,
                    segment_start_ptr(public_data.public_memory.public_segments.add_mod),
                );
        }
        ...
        if let Option::Some(component) = bitwise_builtin.as_snap() {
            component
                .evaluate_constraints_at_point(
                    ...
                    ref interaction_trace_mask_values,
                    random_coeff,
                    segment_start_ptr(public_data.public_memory.public_segments.bitwise),
                );
        }
        ...
        if let Option::Some(component) = mul_mod_builtin.as_snap() {
            component
                .evaluate_constraints_at_point(
                    ...
                    ref interaction_trace_mask_values,
                    random_coeff,
                    segment_start_ptr(public_data.public_memory.public_segments.mul_mod),
                );
        }
        ...
        if let Option::Some(component) = pedersen_builtin.as_snap() {
            component
                .evaluate_constraints_at_point(
                    ...
                    ref interaction_trace_mask_values,
                    random_coeff,
                    segment_start_ptr(public_data.public_memory.public_segments.pedersen),
                );
        }
        ...
        if let Option::Some(component) = poseidon_builtin.as_snap() {
            component
                .evaluate_constraints_at_point(
                    ...
                    ref interaction_trace_mask_values,
                    random_coeff,
                    segment_start_ptr(public_data.public_memory.public_segments.poseidon),
                );
        }
        ...
        if let Option::Some(component) = range_check_96_builtin.as_snap() {
            component
                .evaluate_constraints_at_point(
                    ...
                    ref interaction_trace_mask_values,
                    random_coeff,
                    segment_start_ptr(public_data.public_memory.public_segments.range_check_96),
                );
        }
        ...
        if let Option::Some(component) = range_check_128_builtin.as_snap() {
            component
                .evaluate_constraints_at_point(
                    ...
                    ref interaction_trace_mask_values,
                    random_coeff,
                    segment_start_ptr(public_data.public_memory.public_segments.range_check_128),
                );
        }
        ...
        if let Option::Some(component) = ec_op_builtin.as_snap() {
            component
                .evaluate_constraints_at_point(
                    ...
                    ref interaction_trace_mask_values,
                    random_coeff,
                    segment_start_ptr(public_data.public_memory.public_segments.ec_op),
                );
        }
    }
}
