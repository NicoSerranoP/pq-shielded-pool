...
fn verify_builtins(
    ...
    if let Some(ecdsa_segment_range) = ecdsa_segment_range {
        assert!(ecdsa_segment_range.start_ptr.value == ecdsa_segment_range.stop_ptr.value);
    }
    if let Some(keccak_segment_range) = keccak_segment_range {
...
fn check_builtin(log_size: Option<u32>, segment_range: @Option<SegmentRange>, n_cells: usize) {
...
fn append_segment_range(ref public_claim: Array<u32>, segment: @SegmentRange) {
    public_claim
        .extend(
            array![
                *segment.start_ptr.id, *segment.start_ptr.value, *segment.stop_ptr.id,
                *segment.stop_ptr.value,
            ],
        );
}

fn append_optional_segment_range(ref public_claim: Array<u32>, segment: @Option<SegmentRange>) {
    if let Some(segment) = segment {
        append_segment_range(ref public_claim, segment);
    } else {
        public_claim.extend(array![0_u32, 0_u32, 0_u32, 0_u32]);
    }
}
...
impl PublicDataImpl of PublicDataTrait {
    ...
    fn pack_into_u32s(self: @PublicData) -> (Span<u32>, Span<u32>, Span<u32>) {
        ...
        append_segment_range(ref public_claim, output_ranges);
        append_optional_segment_range(ref public_claim, pedersen);
        append_optional_segment_range(ref public_claim, range_check_128);
        append_optional_segment_range(ref public_claim, ecdsa);
        append_optional_segment_range(ref public_claim, bitwise);
        append_optional_segment_range(ref public_claim, ec_op);
        append_optional_segment_range(ref public_claim, keccak);
        append_optional_segment_range(ref public_claim, poseidon);
        append_optional_segment_range(ref public_claim, range_check_96);
        append_optional_segment_range(ref public_claim, add_mod);
        append_optional_segment_range(ref public_claim, mul_mod);
