...
pub fn run_and_adapt(
    ...
    let (program, mut hints, exec_scopes, public_segment_context) = match program_type {
        ProgramType::Executable => {
            ...
            let (program, hints, public_segment_context) =
                get_program_and_hints_from_executable(&executable, args)?;
    ...
    let mut prover_input = adapt(&cairo_run_program_with_initial_scope(
        &program,
        &cairo_run_config,
        hints.as_mut(),
        exec_scopes,
...
fn get_program_and_hints_from_executable(
...
) -> Result<(Program, Box<dyn HintProcessor>, PublicSegmentContext)> {
    ...
    let entrypoint_builtins = entrypoint.builtins.clone();
    ...
    Ok((program, Box::new(hint_processor), public_segment_context))
}
