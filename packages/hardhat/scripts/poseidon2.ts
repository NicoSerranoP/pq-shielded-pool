// Poseidon2 permutation for BN254, state width t=4.
// Matches poseidon2_permutation([domain, a, b, 0], 4)[0] from Noir stdlib.
// Constants sourced from Poseidon2T4.sol (Barretenberg / provekit-onchain).

const P = 21888242871839275222246405745257275088548364400416034343698204186575808495617n;

// Internal MDS diagonal
const D0 = 0x10dc6e9c006ea38b04b1e03b4bd9490c0d03f98929ca1d7fb56821fd19d3b6e7n;
const D1 = 0x0c28145b6a44df3e0149b3d0a30b3bb599df9756d4dd9b84a86b38cfb45a740bn;
const D2 = 0x00544b8338791518b2c7645a50392798b21f75bb60e3596170067d00141cac15n;
const D3 = 0x222c01175718386f2e2e82eb122789e352e105a3b8fa852613bc534433ee428bn;

// First-half full round constants (rounds 0–3)
const F: bigint[][] = [
  [
...
// Partial round constants (rounds 4–59, lane 0 only)
const PR: bigint[] = [
...
// Second-half full round constants (rounds 60–63)
const G: bigint[][] = [
  [
...
function extMds(s: bigint[]): void {
  const t0 = (s[0] + s[1]) % P;
  const t1 = (s[2] + s[3]) % P;
  const t2 = (s[1] * 2n + t1) % P;
  const t3 = (s[3] * 2n + t0) % P;
  const t1d = (t1 + t1) % P;
  const t4 = (t1d + t1d + t3) % P;
  const t0d = (t0 + t0) % P;
  const t5 = (t0d + t0d + t2) % P;
  s[0] = (t3 + t5) % P;
  s[1] = t5;
  s[2] = (t2 + t4) % P;
  s[3] = t4;
...
function intMds(s: bigint[]): void {
  const sum = (s[0] + s[1] + s[2] + s[3]) % P;
  s[0] = (D0 * s[0] + sum) % P;
  s[1] = (D1 * s[1] + sum) % P;
  s[2] = (D2 * s[2] + sum) % P;
  s[3] = (D3 * s[3] + sum) % P;
...
function permute(s: bigint[]): void {
  extMds(s);
  for (const rc of F) {
    s[0] = sbox5((s[0] + rc[0]) % P);
    s[1] = sbox5((s[1] + rc[1]) % P);
    s[2] = sbox5((s[2] + rc[2]) % P);
    s[3] = sbox5((s[3] + rc[3]) % P);
    extMds(s);
  }
  for (const rc of PR) {
    s[0] = sbox5((s[0] + rc) % P);
...
// poseidon2_permutation([1, left, right, 0], 4)[0] — same as InternalLeanIMTPoseidon2
...
// Rebuild the Merkle root from a leaf map using LeanIMT "bubble-up" logic.
export function buildMerkleRoot(leafMap: Map<number, bigint>, depth: number): bigint {
  if (leafMap.size === 0) return 0n;
  if (depth === 0) return leafMap.get(0) ?? 0n;

  let cur = new Map(leafMap);
  for (let level = 0; level < depth; level++) {
    ...
    for (const [idx] of cur) {
      ...
      seen.add(parent);
      const left = cur.get(parent * 2);
      const right = cur.get(parent * 2 + 1);
      if (left !== undefined && right !== undefined) {
        next.set(parent, poseidon2Hash(left, right));
      } else if (left !== undefined) {
        next.set(parent, left); // odd leaf bubbles up without hashing
      }
    }
    cur = next;
  }
...
// Build a compressed Merkle proof for the leaf at targetIndex.
// Only includes levels where an actual hash occurs (skips "bubble-up" levels).
export function buildMerkleProof(
  leafMap: Map<number, bigint>,
  targetIndex: number,
  depth: number,
): { length: number; indices: boolean[]; siblings: bigint[] } {
  if (depth === 0) return { length: 0, indices: [], siblings: [] };

  // Build all intermediate levels (same logic as buildMerkleRoot, but keep all levels)
  const levels: Map<number, bigint>[] = [new Map(leafMap)];
  for (let level = 0; level < depth; level++) {
    ...
    for (const [idx] of cur) {
      ...
      if (left !== undefined && right !== undefined) {
        ...
        next.set(parent, left);
      }
    }
    levels.push(next);
  }

  // Walk the proof path; only emit entries where a sibling exists (real hash operation)
  const indices: boolean[] = [];
  const siblings: bigint[] = [];
  let curIdx = targetIndex;

  for (let level = 0; level < depth; level++) {
    const isRight = (curIdx & 1) === 1;
    const sibIdx = isRight ? curIdx - 1 : curIdx + 1;
    const sibling = levels[level].get(sibIdx);
  ...
  return { length: siblings.length, indices, siblings };
}
