...
PROOF_ARGS_PATH="${RECURSIVE_PROOF_ARGS:-$ROOT_DIR/packages/cairo-merkle-poseidon/target/local-proofs/merkle-poseidon-proof.blake-canonical.cairo-serde.array-args.json}"
...
if [[ ! -f "$PROOF_ARGS_PATH" ]]; then
  ...
  echo "Run: corepack yarn cairo:merkle-poseidon:prove-recursive" >&2
  exit 1
fi
...
(
  cd "$VERIFIER_DIR"
  "$SCARB_BIN" --profile proving build --package stwo_cairo_verifier --features "$FEATURES"
  "$SCARB_BIN" --profile proving execute --no-build \
